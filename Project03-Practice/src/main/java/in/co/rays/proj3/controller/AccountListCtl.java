...
public class AccountListCtl extends BaseCtl {
	...
	protected BaseDTO populateDTO(HttpServletRequest request) {
		...
		dto.setAccNumber(request.getParameter("accNumber"));
		dto.setHolderName(request.getParameter("holderName"));
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException,DatabaseException {
		...
		try {
			List<AccountDTO> list=model.search(dto,pageNo,pageSize);
			List<AccountDTO> next=model.search(dto,pageNo+1,pageSize);
