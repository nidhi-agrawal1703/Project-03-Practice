...
@WebServlet(name="AllowListCtl",urlPatterns= {"/ctl/AllowListCtl"})
public class AllowListCtl extends BaseCtl{
	
	@Override
	protected BaseDTO populateDTO(HttpServletRequest request) {
		AllowDTO dto=new AllowDTO();
		
		dto.setAllowCode(request.getParameter("allowCode"));
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException,DatabaseException {
		...
		try {
			List<AllowDTO> list=model.search(dto,pageNo,pageSize);
			List<AllowDTO> next=model.search(dto,pageNo+1,pageSize);
		...
		try {
			if(OP_SEARCH.equalsIgnoreCase(op)||"Next".equalsIgnoreCase(op)||"Previous".equalsIgnoreCase(op)) {
				...
				if(ids!=null && ids.length>0) {
					AllowDTO deletedto=new AllowDTO();
					for(String id:ids) {
	...
	protected String getView() {
		return ORSView.ALLOW_LIST_VIEW;
	}
